...
export async function fetchTransactionsFromProgramId(programId: string, connectionUrl: string, fetchTransactionsParam: FetchTransactionsParam) {
    ...
    while (true) {
        ...
        const parsedTransactions = await connection.getParsedTransactions(transactionSignatures.map(signatureInfo => signatureInfo.signature), 'finalized');

        const filteredTransactions = parsedTransactions
            .filter(parsedTransaction => parsedTransaction !== null)
            .filter(parsedTransaction => fetchTransactionsParam.startTime ? parsedTransaction?.blockTime && parsedTransaction.blockTime >= fetchTransactionsParam.startTime : true)
            .filter(parsedTransaction => fetchTransactionsParam.endTime ? parsedTransaction?.blockTime && parsedTransaction.blockTime <= fetchTransactionsParam.endTime : true);
